...
export async function exportReportAsPDF(elementId, tokenName = 'Token') {
  ...
  if (!element) {
    console.error('[Dolphin Research] PDF export: Could not find .report-main element');
    return;
  }

  const date = new Date().toISOString().split('T')[0];
  const safeName = tokenName.replace(/[^a-zA-Z0-9]/g, '_');
  const filename = `Dolphin_Research_Report_${safeName}_${date}.pdf`;
  ...
  try {
    ...
    console.error('[Dolphin Research] PDF export error:', err);
    throw err;
  }
}
